...
export function useOnScreen(ref, rootMargin = '0px', options) {
...
export function useScrollSnap(plusHeight = 0) {
  const container = useRef(null);
  useEffect(() => {
    const gatsbyContainer = document.getElementById('__next');
...
export const useElementSize = (ref = null) => {
  const [elementSize, setElementSize] = useState({
    width: 0,
    height: 0,
  });

  const elementRef = ref || useRef();
...
export function useHasEntered(ref, rootMargin = '0px') {
  // State and setter for storing whether element is visible
...
export function useDimensions() {
  const ref = useRef();
  const [dimensions, setDimensions] = useState({ width: 0, height: 0, top: 0, left: 0, right: 0, bottom: 0, x: 0, y: 0 });
  useEffect(() => {
